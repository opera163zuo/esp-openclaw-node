...
- **门户可选鉴权。** 设置 `portal_password`（留空或 8–63 字符）后，所有 API 请求都要求 HTTP Basic
  认证；浏览器会原生弹出凭据框，所以 Web UI 不需要自己的登录页。用户名被忽略，只校验密码。
  留空是默认值——首次配网必须能进，否则会把用户锁在门外。
- 门户在 **STA 接口（局域网）上同样可达**，仅靠 AP 密码保护不了 LAN 访问。设备部署在共享网络时
  建议设置 `portal_password`。
