...
| `boards/` | `esp-board-manager` board definitions (`idf.py gen-bmgr-config -b <board> -c boards`) |
...
mcp_server_point does not use the Agent event router or JSON routing rules that the de-agentification removed. MCP clients call **`lua.run_script`**, **`lua.run_script_async`**, **`lua.list_jobs`**, **`lua.get_job`**, **`lua.stop_job`**, and **`lua.stop_all_jobs`**; scripts must already exist under `scripts/` (for example under `fatfs_image/`).
...
after running `idf.py gen-bmgr-config -b <board> -c boards`.
...
| Enabled caps | **`CONFIG_APP_CLAW_CAP_LUA=y`**. The MCP server is not an `app_claw` capability — `main.c` starts `cap_mcp_server` directly. |
| Disabled caps | **`CONFIG_APP_CLAW_CAP_FILES=n`**, the only other capability group left. The groups the de-agentification removed (core, memory, event router, IM platforms, scheduler, router, session, skill, system, web search, LLM inspect, MCP client) no longer have Kconfig symbols, so they are not listed. |
| Lua | `CONFIG_APP_CLAW_LUA_MODULE_BLE_HID=n` (no BT stack) |
...
From `application/mcp_server_point`, configure the board first. The custom board
lives in this app's own `boards/` tree, which `gen-bmgr-config` does **not** scan
by default — only the board-manager component's boards and the project's
`components/` directory. Passing the customer path is therefore mandatory, and
omitting it fails with `Board "esp_Ditto" not found`:

```bash
idf.py gen-bmgr-config -b esp_Ditto -c boards   # once, and again after switching boards
idf.py build
...
- **`application/edge_agent/README.md`** — the full Native Node app (serial CLI, display, files and Lua capabilities).
